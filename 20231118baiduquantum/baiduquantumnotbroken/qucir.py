...
for i in range(len(loss_components)):
    if i == 0:
        ...
        print('The theoretical ground state energy is: ', np.linalg.eigh(H)[0][i])
    else:
        ...
        print('The theoretical {} excited state energy is: {}'.format(
            output_ordinalvalue(i), np.linalg.eigh(H)[0][i])
        )

# 储存训练结果到 output 文件夹
os.makedirs("output", exist_ok=True)
savez("./output/summary_data", iter = summary_iter, energy=summary_loss_components)

cir.plot(
    save_path=r"D:\pythonstarter\baiduquantumnotbroken\circuit.png",  # 保存图像的路径
    dpi=300,  # 分辨率
    show=False,  # 是否显示图像
...
plt.savefig(r"D:\pythonstarter\baiduquantumnotbroken\ground_energy.png")